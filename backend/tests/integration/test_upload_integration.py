...
from app.services.indexing_progress import mark_indexing_failed, mark_indexing_ready
from tests.factories import ChatSessionFactory


def _complete_ingest(db: Session, document_id: int, chunks: int = 3) -> None:
    mark_indexing_ready(db, document_id, chunks)


def _failed_ingest(db: Session, document_id: int, message: str = "boom") -> None:
    mark_indexing_failed(db, document_id, message)


def test_upload_txt_document(auth_client, db_session):
    session = ChatSessionFactory(user=auth_client.auth_user, title="Upload Chat")

    with patch("app.api.upload_routes.run_ingest_document_record") as mock_ingest:
        mock_ingest.side_effect = lambda document_id: _complete_ingest(db_session, document_id, 3)
...
def test_upload_pdf_document(auth_client, db_session):
    session = ChatSessionFactory(user=auth_client.auth_user, title="PDF Upload")

    with patch("app.api.upload_routes.run_ingest_document_record") as mock_ingest:
        mock_ingest.side_effect = lambda document_id: _complete_ingest(db_session, document_id, 4)
        response = auth_client.post(
            f"/upload?session_id={session.id}",
            headers=auth_client.auth_headers,
...
def test_upload_docx_document(auth_client, db_session):
    session = ChatSessionFactory(user=auth_client.auth_user, title="DOCX Upload")

    with patch("app.api.upload_routes.run_ingest_document_record") as mock_ingest:
        mock_ingest.side_effect = lambda document_id: _complete_ingest(db_session, document_id, 5)
        response = auth_client.post(
            f"/upload?session_id={session.id}",
            headers=auth_client.auth_headers,
            files={
...
def test_upload_markdown_document(auth_client, db_session):
    session = ChatSessionFactory(user=auth_client.auth_user, title="Markdown Upload")

    with patch("app.api.upload_routes.run_ingest_document_record") as mock_ingest:
        mock_ingest.side_effect = lambda document_id: _complete_ingest(db_session, document_id, 2)
        response = auth_client.post(
            f"/upload?session_id={session.id}",
            headers=auth_client.auth_headers,
...
def test_upload_processing_failure_returns_error(auth_client, db_session):
    session = ChatSessionFactory(user=auth_client.auth_user, title="Failure Upload")

    with patch("app.api.upload_routes.run_ingest_document_record") as mock_ingest:
        mock_ingest.side_effect = lambda document_id: _failed_ingest(db_session, document_id, "boom")
        response = auth_client.post(
            f"/upload?session_id={session.id}",
            headers=auth_client.auth_headers,
...
def test_delete_document_by_id(auth_client, db_session):
...
def test_multiple_uploads(auth_client, db_session):
